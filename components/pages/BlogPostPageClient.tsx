...
const BlogPostHeader: React.FC<{post: BlogPost; formattedDate: string; t: (key: string) => string}> = ({post, formattedDate, t}) => (
  <div className="mb-8">
    ...
    {post.subtitle && <h2 className="text-xl md:text-2xl text-gray-600 mb-6 font-medium">{post.subtitle}</h2>}
...
const CallToAction: React.FC<{t: (key: string) => string}> = ({t}) => (
...
const SocialSharing: React.FC<{post: BlogPost; t: (key: string) => string}> = ({post, t}) => (
...
const PostNavigation: React.FC<{nextPost: {slug: string; title: string} | null; prevPost: {slug: string; title: string} | null}> = ({nextPost, prevPost}) => {
  const t = useTranslations('Blog');

  return (
    <div className="flex space-x-4">
      {prevPost && (
        <Link href={`/${prevPost.slug}`} className="flex-1 bg-gray-50 hover:bg-gray-100 p-4 rounded-lg transition-colors">
          <div className="text-sm text-gray-500 mb-1">{'<-'} {t('previous')}</div>
          <div className="font-medium text-gray-900">{prevPost.title}</div>
        </Link>
      )}
      {nextPost && (
        <Link href={`/${nextPost.slug}`} className="flex-1 bg-gray-50 hover:bg-gray-100 p-4 rounded-lg transition-colors text-right">
...
export default function BlogPostPageClient({post, nextPost, prevPost}: BlogPostPageProps) {
  const t = useTranslations('Blog');
