...
export default function Globe() {
  ...
  useEffect(() => {
    ...
    const createPopupTimelines = () => {
      ...
      popupOpenTl.current = gsap.timeline({
        paused: true
      })
        .to(state.pointer.material, {
          duration: 0.2,
          opacity: 1,
        }, 0)
      ...
      popupCloseTl.current = gsap.timeline({
        paused: true
      })
        .to(state.pointer.material, {
          duration: 0.3,
    ...
    const drawPopupConnector = (startX: number, startY: number, midX: number, midY: number, endX: number, endY: number) => {
      if (!overlayCtx) return;

      overlayCtx.strokeStyle = '#000000';
      overlayCtx.lineWidth = 3;
      overlayCtx.lineCap = 'round';
      overlayCtx.clearRect(0, 0, containerRef.current?.offsetWidth || 0, containerRef.current?.offsetHeight || 0);
    ...
    const showPopupAnimation = (lifted: boolean) => {
      if (!state.pointer || !popupCloseTl.current || !popupOpenTl.current) return;

      if (lifted) {
        let positionLifted = state.pointer.position.clone();
    ...
    const updateOverlayGraphic = () => {
      if (!state.pointer || !state.globe || !state.camera || !containerRef.current) return;

      let activePointPosition = state.pointer.position.clone();
      activePointPosition.applyMatrix4(state.globe.matrixWorld);
      const activePointPositionProjected = activePointPosition.clone();
      activePointPositionProjected.project(state.camera);
      
      state.coordinates2D[0] = (activePointPositionProjected.x + 1) * containerRef.current.offsetWidth * 0.5;
      state.coordinates2D[1] = (1 - activePointPositionProjected.y) * containerRef.current.offsetHeight * 0.5;
      ...
      if (state.controls) {
        ...
        if (activePointPosition.z > -1) {
          ...
          const curveMidY = popupY + (upDown ? -0.5 : 0.1) * state.coordinates2D[1];
    ...
    const addCanvasEvents = () => {
      ...
      const updateMousePosition = (eX: number, eY: number) => {
        ...
        state.mouse.x = ((eX - containerRef.current.offsetLeft) / containerRef.current.offsetWidth) * 2 - 1;
        state.mouse.y = -((eY - containerRef.current.offsetTop) / containerRef.current.offsetHeight) * 2 + 1;
      };
    ...
    const render = () => {
      if (!state.renderer || !state.scene || !state.camera || !state.mapMaterial) return;

      state.mapMaterial.uniforms.u_time_since_click.value = state.clock.getElapsedTime();
      checkIntersects();
      if (state.pointer) {
    ...
    const init = async () => {
      ...
      state.earthTexture = await textureLoader.loadAsync('https://ksenia-k.com/img/earth-map-colored.png');
    ...
    <div className={styles.container} ref={containerRef}>
      <div className={styles.page}>
        <div className={styles.globeWrapper}>
          ...
          <canvas 
            ref={canvas2DRef}
            className={styles.globe2DOverlay}
          />
          <div className={styles.globePopupOverlay}>
            <div 
              ref={popupRef}
              className={styles.globePopup} 
            />
          </div>
        </div>
      </div>
